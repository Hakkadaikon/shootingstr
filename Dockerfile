...
RUN apt-get -y update && \
    apt-get -y upgrade && \
    apt-get -y install make wget zip unzip git build-essential && \
...
RUN wget ${CMAKE_PKG_URL} && sh $(basename ${CMAKE_PKG_URL}) --prefix=/usr/local --exclude-subdir

# Install ninja
ARG NINJA_PKG_URL=http://archive.ubuntu.com/ubuntu/ubuntu/pool/universe/n/ninja-build/ninja-build_1.12.1-1_amd64.deb
RUN wget ${NINJA_PKG_URL} && dpkg -i $(basename ${NINJA_PKG_URL})
...
RUN wget ${MESON_PKG_URL} && dpkg -i $(basename ${MESON_PKG_URL})
